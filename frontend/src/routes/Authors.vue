<template>
	...
	<div>
		...
		<el-breadcrumb separator-class="el-icon-arrow-right">
			...
			<el-breadcrumb-item>Authors</el-breadcrumb-item>
		</el-breadcrumb>
		...
		<el-table :data="tableData" stripe  style="width: 100%" @sort-change="sortChange" v-loading="isLoading" empty-text="Nothing here yet">
			...
			<el-table-column prop="firstName" sortable label="First Name" width="180"></el-table-column>
			<el-table-column prop="lastName" sortable label="Last Name" width="180"></el-table-column>
...
export default {
	...
	methods: {
		...
		commitAdd: async function() {
			this.addFormVisible = false;
			let savedOnServer = await this.$api.getCollection(this.collectionName).post(this.addItem);
		...
		commitEdit: async function() {
			this.editFormVisible = false;
			let savedOnServer = await this.$api.getCollection(this.collectionName).edit(this.editItem);
			this.$notify({
					title: 'Success',
